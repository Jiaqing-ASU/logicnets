...
from training_methods import test, train, train_bagging, train_adaboost, test_predictions_return
        ...
        if "ensemble_method" in config:
            if config["ensemble_method"] == "averaging":
                ...
                for model in ensemble_ckpt_list:
                    ...
                    df.to_csv(os.path.join(experiment_dir, f"{args.experiment_name}_model_{count}_predictions.csv"), index=False)
            elif config["ensemble_method"] == "bagging":
                print("Bagging ensemble method")
                if "independent" not in config:
                    config["independent"] = False # Default
