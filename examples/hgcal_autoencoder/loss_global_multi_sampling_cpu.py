...
def main(args):
    ...
    if evaluate_model:
        ...
        ###############################################################################
        # Evaluate the loss along points on different sizes of the grid
        ###############################################################################

        for i in range(len(args.points_box)):
            POINTS = args.points_box[i]

            this_loss_coordinates = loss_coordinates[:POINTS]
            print("Generated coordinates: ", this_loss_coordinates.shape)
            ...
            for j in tqdm(range(POINTS), desc="Calculating sampling loss values in the subspace"):
                ...
                next_pos = tuple(this_loss_coordinates[j])
            ...
            if DIM == 2 and args.visualize:
                x = this_loss_coordinates[:, 0]
                y = this_loss_coordinates[:, 1]
            ...
            # save the coordinates
            np.save('loss_landscapes_global/' + str(args.experiment_name) + '_hessian_loss_landscape_' + f'boxsize{args.box_size}_max_pct{args.vmax_pct}_dim{DIM}_points{POINTS}_coords.npy', this_loss_coordinates)
            # save the loss with its corresponding model coordinates into a json
            with open('loss_landscapes_global/' + str(args.experiment_name) + '_hessian_loss_landscape_' + f'boxsize{args.box_size}_max_pct{args.vmax_pct}_dim{DIM}_points{POINTS}.json', "w") as f:
                json.dump({"loss_values": data_matrix.tolist(), "loss_coordinates": this_loss_coordinates.tolist(), "numbers of models": len(models)}, f)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dim', default=5, help='dimension for hessian loss values calculation')
    parser.add_argument('--dimfilter', default=5, help='dimension for hessian loss values calculation')
    parser.add_argument('--steps', default=50, help='steps for hessian loss values calculation')
    parser.add_argument('--points', default=100000, help='total points while sampling for hessian loss values calculation')
    parser.add_argument('--points-box', nargs='+', default=[5000,10000,25000,50000,75000,100000], help='bits to be used for the quantization')
