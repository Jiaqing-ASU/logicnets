...
class LossPINN(Metric):
    """ Computes a specified loss function over specified input-output pairs. """
    def __init__(self, inputs: torch.Tensor, target: torch.Tensor):
        super().__init__()
        self.inputs = inputs
        self.target = target

    def __call__(self, model, model_wrapper: ModelWrapper) -> float:
        if torch.is_grad_enabled():
            model.optimizer.zero_grad()
            u_pred = model_wrapper(torch.cat([self.inputs, self.target], dim=1))
        ...
        if model.loss_style == 'mean':
            loss_u = torch.mean((self.target - u_pred) ** 2)
        ...
        return loss_u + loss_b + model.L*loss_f
