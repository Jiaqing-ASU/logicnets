...
import loss_landscapes_pinn.model_interface.model_parameters as model_parameters
from loss_landscapes_pinn.model_interface.model_wrapper import ModelWrapper, wrap_model
from loss_landscapes_pinn.model_interface.model_parameters import rand_u_like, orthogonal_to
from loss_landscapes_pinn.metrics.metric import Metric
...
def random_n_directions(model: typing.Union[torch.nn.Module, ModelWrapper], metric: Metric, dim=3, distance=1, steps=20,
                        points = 20, normalization='filter', deepcopy_model=False) -> np.ndarray:
    """
    Returns the computed value of the evaluation function applied to the model or agent along a 
    ...
    for j in tqdm.tqdm(range(points), desc="Calculating sampling loss values in the subspace"):
        ...
        data_matrix[j] = metric(model_start_wrapper)
        # print("The loss value at " + str(next_pos) + " is " + str(data_matrix[next_pos]))
        # set the parameters back to the original start point
        for i in range(dim):
    ...
    return np.array(data_matrix), loss_coordinates

# noinspection DuplicatedCode
def random_plane_pinn(model: typing.Union[torch.nn.Module, ModelWrapper], metric: Metric, distance=1, steps=20,
                 normalization='filter', deepcopy_model=False) -> np.ndarray:
    ...
    directions = [dir_one, dir_two]

    return directions

def random_n_dirctions_pinn(model: typing.Union[torch.nn.Module, ModelWrapper], metric: Metric, dim=3, distance=1, steps=20,
                 normalization='filter', deepcopy_model=False) -> np.ndarray:
